...
AS orders_count,
    ...
    <div id="sidebar-menu">
      ...
      <ul class="metismenu list-unstyled" id="side-menu">
        ...
        <li>
          <a href="{{ route('admin.product.questions') }}" class="waves-effect d-flex justify-content-between">
            <span>
              <i class="bx bx-detail"></i>
              {{ __('admin.product_questions') }}
            </span>
            @if ($counts->questions_count > 0)
              <span class="badge bg-warning rounded-circle d-inline-flex align-items-center justify-content-center"
        ...
        <li>
          <a href="{{ route('admin.refund.index') }}" class="waves-effect d-flex justify-content-between">
            <span>
              <i class="bx bx-truck"></i>
              {{ __('admin.refund_list') }}
            </span>
            @if ($counts->refunds_count > 0)
              <span class="badge bg-danger rounded-circle d-inline-flex align-items-center justify-content-center"
                style="width: 20px; height: 20px; font-size: 11px;">{{ $counts->refunds_count }}</span>
            @endif
          </a>
        </li>
        <li>
        ...
        <li>
          <a href="{{ route('promo-codes.index') }}" class="waves-effect">
            ...
            <span key="t-vendors">{{ __('admin.promo_codes') }}</span>
          </a>
        </li>
