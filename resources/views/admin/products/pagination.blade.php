  ...
  <table class="dataTable dt-responsive table table-hover table-nowrap align-middle mb-0">
    <thead class="table-light">
      <tr>
        ...
        <th class="align-middle">{!! sorting('sku_number', __('admin.sku_number'), $sortOrder, $sortEntity) !!}</th>
        <th class="align-middle">{!! sorting('no_of_pieces_available', __('admin.no_of_pieces_available'), $sortOrder, $sortEntity) !!}</th>
        <th class="align-middle" data-priority="1">{!! sorting('status', __('admin.status'), $sortOrder, $sortEntity) !!}</th>
...
@endif
            ...
            <td>
              ...
              <a style="cursor: pointer; color:red" title="{!! __('admin.delete') !!}"
                ...
                data-url="{!! route('products.destroy', $row->id) !!}" data-confirm="{!! __('admin.delete_confirmation_message') !!}"><i
    ...
    <tr>
      <td colspan="11">
        <button type="button" class="btn btn-primary btn-xs __toggle_all" data-route="{!! route('products.toggle-all-status', 1) !!}"><i
            class="fa fa-check"></i> {!! __('admin.activate') !!}</button>
        <button type="button" class="btn btn-secondary btn-xs __toggle_all" data-route="{!! route('products.toggle-all-status', 0) !!}"><i
